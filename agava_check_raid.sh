...
if test $# -eq 0
then
    echo "Usage: agava_check_raid -H HOSTNAME [-C COMMUNITY] [-t TIMEOUT]"
    exit 3
fi
...
status=`snmpget -Oqv -v1 $snmpcommunity $timeout $hostaddress UCD-SNMP-MIB::extResult.1`
message=`snmpget -Oqv -v1 $snmpcommunity $timeout $hostaddress UCD-SNMP-MIB::extOutput.1`
